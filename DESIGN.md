...
每个需要玩家分级的功能（respawn、pvp 等）必须包含以下三个组别：

| 组别 | 配置位置 | 匹配方式 | 说明 |
|------|---------|---------|------|
| **default** | `权限组.default` | 无条件兜底 | 没有任何权限节点的玩家 |
| **独立OP组** | `OP组` | `player.isOp()` + `启用独立OP组: true` | OP 玩家跳过权限匹配 |
| **vip** | `权限组.vip` | 权限节点 `模块名.vip` | 示例配置写在该组下 |

### 新功能原则

- 新加功能默认**不影响 default 组和 OP 组**，除非专门说明
- 新功能的示例配置写在 **vip 组**下，不修改 default 和 OP 组
- 所有功能默认关闭（`启用: false`），由服主按需开启
...
效果名支持常见别名（`mining_fatigue` → `SLOW_DIGGING`、`nausea` → `CONFUSION`）。
